...
SQL (T-SQL on SQL Server) · Python (pandas, Matplotlib, Seaborn) · Statistics (A/B testing, cohort analysis, RFM segmentation)
...
|---|---:|---|
| Order system migrated on 2025-01-01: dates switched from `DD/MM/YYYY HH:MI` to ISO | 18,203 | Parse each format with `TRY_CONVERT` styles 103 and 120 |
...
flowchart LR
    ...
    F --> X["outputs/<br/>CSV exports"]
    X --> H["ab_test.py<br/>eda_charts.py"]
```

Three layers keep each step honest: `raw` holds the extracts untouched, `clean` holds typed and standardised tables, and `mart` holds the analysis tables and one view per business question. All three live in a SQL Server database called `Kaarobar`.
...
## How to run
Requires SQL Server (tested on 2025 Express) and Python 3.11 or later. Run from the project root:

```bash
python -m pip install -r requirements.txt
python src/generate_data.py   # about 40 seconds; writes data/raw/
python src/run_pipeline.py    # builds the Kaarobar database, runs every SQL file, exports results to outputs/
python src/ab_test.py         # A/B test statistics and chart
python src/eda_charts.py      # charts to images/
```

On Windows, double-clicking `run_all.bat` does all of this in one step.

`run_pipeline.py` runs all SQL through `sqlcmd`, which it finds on the PATH or in its usual install folder. It connects with Windows authentication, so there is no password to type, and creates the database if it does not exist. Override the defaults with the `SQLSERVER` and `SQLDATABASE` environment variables (default `localhost\SQLEXPRESS` and `Kaarobar`).

The data is seeded, so every run reproduces the same numbers. Query results land in `outputs/` as CSV, and any SQL Server client, such as SQL Server Management Studio or the VS Code SQL Server extension, can query the `Kaarobar` database directly.

To run the SQL by hand instead, create a `Kaarobar` database and run the files in order from the project root. `00_load_sqlserver.sql` needs the data folder passed in, for example `sqlcmd -S localhost\SQLEXPRESS -E -d Kaarobar -i sql/00_load_sqlserver.sql -v DataDir="C:\path	o\data
aw\\"`.
...
└── src/
    ...
    ├── run_pipeline.py    builds the database with sqlcmd and exports results
    ├── ab_test.py         significance tests and sample ratio check
    └── eda_charts.py      exploratory charts
```

## Techniques used
- **SQL:** CTEs, window functions (`ROW_NUMBER`, `LAG`, `NTILE`, `SUM() OVER`), filtered aggregates, lookup-table standardisation, regex cleaning, deduplication, collation control for exact text matching, data quality tests
...
- **Python:** pandas, Matplotlib, Seaborn; sqlcmd driven from Python with subprocess
- **Tools:** SQL Server 2025 Express, sqlcmd, Git
...
- RFM scores use quintiles; many customers share the same order count, so ties are split by customer id rather than by behaviour.
