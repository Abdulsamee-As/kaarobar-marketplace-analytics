...
SQL (PostgreSQL, also runs on DuckDB) · Python (pandas, Matplotlib, Seaborn) · Statistics (A/B testing, cohort analysis, RFM segmentation)
...
The SQL is written in PostgreSQL syntax and tested end to end on both PostgreSQL 18 and DuckDB, with identical results. The Python scripts use DuckDB because it needs no database server. To build the database in PostgreSQL instead, run `sql/00_load_postgres.sql` with `psql` from the project root, then files 01 to 05 in order.
