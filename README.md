...
SQL (PostgreSQL) · Python (pandas, Matplotlib, Seaborn) · Statistics (A/B testing, cohort analysis, RFM segmentation)
...
Three layers keep each step honest: `raw` holds the extracts untouched, `clean` holds typed and standardised tables, and `mart` holds the analysis tables and one view per business question. All three live in a PostgreSQL database called `kaarobar`.
...
## How to run
Requires PostgreSQL (tested on version 18) and Python 3.11 or later. Run from the project root:
...
To run the SQL by hand instead, create the `kaarobar` database and run `sql/00_load_postgres.sql` with `psql` from the project root (its `\copy` commands read `data/raw/`), then files 01 to 05 in order.
...
└── src/
    ...
    ├── run_pipeline.py    builds the database with psql and exports results
...
- **Python:** pandas, Matplotlib, Seaborn; psql driven from Python with subprocess
- **Tools:** PostgreSQL 18, psql, Git
